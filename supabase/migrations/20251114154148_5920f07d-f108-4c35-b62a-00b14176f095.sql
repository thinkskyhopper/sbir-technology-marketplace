-- Create enum type for listing type
CREATE TYPE listing_type AS ENUM ('Contract', 'IP', 'Contract & IP');

-- Add listing_type column to sbir_listings table with default value
ALTER TABLE sbir_listings 
ADD COLUMN listing_type listing_type NOT NULL DEFAULT 'Contract';

-- Drop the existing function first
DROP FUNCTION IF EXISTS public.get_public_listings_with_profiles();

-- Recreate the RPC function with the new field
