-- Create corrected column-level RLS policies for sbir_listings to restrict admin-only data
...
COMMENT ON POLICY "Public can view active and sold listings (safe columns only)" ON public.sbir_listings IS 'Allows public access to Active/Sold listings but application layer enforces column restrictions for non-admins';